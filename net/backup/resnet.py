...
class ResNet():
    ...
    def BasicBlock(self, inputs, filters, strides=1, is_training=False, use_bn=True):
        expansion = 1
        conv1_bn_relu = _conv(inputs, filters, [3,3], strides, 'SAME', activation=tf.nn.relu, is_training=self.is_training, use_bn=self.use_bn, name="conv"+str(self.counter))
        self.counter += 1
        conv2_bn = _conv(conv1_bn_relu, filters, [3,3], 1, 'SAME', activation=None, is_training=self.is_training, use_bn=self.use_bn, name="conv"+str(self.counter))
        self.counter += 1
        if strides != 1 or self.inplanes != filters*expansion:
            inputs = _conv(inputs, filters, [1,1], strides, 'VALID', activation=None, is_training=self.is_training, use_bn=self.use_bn, name="conv"+str(self.counter))
            self.counter += 1
    ...
    def _layer0(self, inputs, filters, kernel_size=(7, 7)):
        outputs = _conv(inputs, filters, [7,7],  2, 'SAME',  activation=tf.nn.relu, is_training=self.is_training, use_bn=self.use_bn, name="conv"+str(self.counter))
        self.counter += 1
    ...
    def forward(self, inputs):
        ...
        return self.layer1, self.layer2, self.layer3,self.layer4, self.first_layer
